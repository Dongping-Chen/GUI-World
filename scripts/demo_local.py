...
parser.add_argument("--qs", type=str, default=None)
parser.add_argument("--video_path", type=str, default=None)
parser.add_argument("--keyframes", type=int, default=None)
args = parser.parse_args()
# load stage2 model
cfg.model.vision_encoder.num_frames = args.keyframes
...
state_dict = torch.load(args.ckpt_path, "cuda")
...
def answer(conv, model, img_list, do_sample=True, max_new_tokens=200, num_beams=1, min_length=1, top_p=0.9,
               repetition_penalty=1.2, length_penalty=1.2, temperature=1.0, answer_prompt=None, print_res=False):
    stop_words_ids = [
...
# num_frame = 8
num_frame = args.keyframes
# resolution = 384
resolution = 224
try:
    vid, msg = load_video(args.video_path, num_segments=num_frame, return_msg=True, resolution=resolution)
...
img_list = []
with torch.no_grad():
    image_emb, _ = model.encode_img(video, "Watch the video and follow the user's instruction.")
...
llm_message = answer(conv=chat, model=model, do_sample=False, img_list=img_list, max_new_tokens=1024, print_res=True)[0]
print(llm_message)
